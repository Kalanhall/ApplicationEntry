...
open class FlashyTabBar: UITabBar {
    ...
    private func reloadAnimations() {
        buttons.forEach { (button) in
            button.selectAnimation = TabItemSelectAnimation(duration: 0.5 / animationSpeed)
            button.deselectAnimation = TabItemDeselectAnimation(duration: 0.5 / animationSpeed)
        }
    }

    private func button(forItem item: UITabBarItem) -> TabBarButton {
        let button = TabBarButton(item: item, hasDot: hasDotView)
        button.tintColor = tintColor
        if (imageTintColor != nil) { button.imageTintColor = imageTintColor }
